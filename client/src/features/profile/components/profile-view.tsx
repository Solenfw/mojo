...
import {
  ...
  MapPin, Mic, PenTool, ShieldCheck, Sparkles, Star, Target, Trophy, User, Zap
} from 'lucide-react';
...
const ACHIEVEMENTS = [
  { title: 'Kana Certified', description: 'Completed hiragana and katakana foundations.', icon: Trophy },
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-5">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2 mb-2">
              ...
              <Badge className="bg-accent text-white border-none font-bold">Pro Member</Badge>
            </div>
            <h2 className="text-3xl font-bold tracking-tighter text-primary">{profile.name}</h2>
            <p className="text-muted-foreground">Target: JLPT {profile.target_level}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: 'Current Level', value: profile.current_level, helper: 'Foundation', icon: BookOpen },
          { label: 'Active Streak', value: `${profile.streak} Days`, helper: 'Daily practice', icon: Flame },
          { label: 'Total XP', value: profile.xp.toLocaleString(), helper: 'Top 18% this month', icon: Zap },
          ...
          <CardContent className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="rounded-xl bg-primary text-white p-5 relative overflow-hidden">
                ...
                <p className="text-lg font-bold">Business Japanese</p>
              </div>
          ...
          <CardContent>
            <div className="bg-white/10 rounded-xl p-6 backdrop-blur-md border border-white/10 space-y-5">
              ...
              <div className="space-y-3">
                {WEEKLY_PLAN.slice(0, 5).map((item) => (
