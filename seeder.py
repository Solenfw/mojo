...
from server.app.db.models import t_user_roles
...
async def _seed_all_lessons(db, course: models.Courses) -> None:
    ...
    for category in categories:
        for source_file, source in _load_jsons(category):
            ...
            # Store the raw JSON content so the frontend can parse UI elements
    ...
    async with SessionLocal() as db:
        ...
        for user_data in users_to_seed:
            ...
            if not user:
                ...
                await db.flush()
                await db.execute(
                    insert(t_user_roles).values(
                        user_id=user.id,
                        role_id=seeded_roles[user_data["role_code"]].id
                    )
                )
